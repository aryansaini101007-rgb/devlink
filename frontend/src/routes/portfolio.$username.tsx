...
function PortfolioPage() {
  ...
  const renderModernNeon = () => (
    <div className="min-h-screen bg-[#07090e] text-slate-100 font-sans relative overflow-hidden pb-20">
      {/* Glow auras */}
      <div className="absolute top-[-20%] left-[-10%] w-[500px] h-[500px] rounded-full filter blur-[120px] opacity-10 pointer-events-none" style={{ backgroundColor: activeAccent.hex }} />
      <div className="absolute bottom-[20%] right-[-10%] w-[600px] h-[600px] rounded-full filter blur-[150px] opacity-[0.08] pointer-events-none" style={{ backgroundColor: activeAccent.hex }} />
      ...
      <header className="border-b border-slate-800 bg-[#07090e]/80 backdrop-blur-md sticky top-0 z-30">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link to="/dashboard" className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-slate-100 transition-colors">
            <ArrowLeft size={14} /> Back to DevLink
          </Link>
          <div className="flex items-center gap-3">
            <span className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full ${activeAccent.badge} border`}>
      ...
      <main className="max-w-4xl mx-auto px-4 pt-12 space-y-16 relative z-10">
        ...
        <section className="flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left bg-slate-900/40 border border-slate-800/80 p-8 rounded-2xl backdrop-blur-md">
          <div className="relative shrink-0">
            <img src={b.avatar} alt={b.name} className="w-28 h-28 rounded-2xl border-2 border-slate-700 bg-slate-800" />
            <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-4 border-[#07090e] bg-success`} />
          </div>
          <div className="space-y-4 flex-1">
            <div>
              ...
              <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 mt-3 text-xs text-slate-400">
                <span className="flex items-center gap-1"><MapPin size={13} /> {b.country}</span>
                <span className="flex items-center gap-1"><Calendar size={13} /> Active Builder</span>
                <span className="flex items-center gap-1"><Globe size={13} /> devlink.io/portfolio/{b.handle}</span>
              </div>
            </div>
            ...
            <div className="flex justify-center md:justify-start items-center gap-2 pt-2">
              <a href="https://github.com" target="_blank" rel="noreferrer" className="p-2 bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700 text-slate-300 hover:text-white rounded-lg transition-colors">
                <Github size={16} />
              </a>
              <a href="https://twitter.com" target="_blank" rel="noreferrer" className="p-2 bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700 text-slate-300 hover:text-white rounded-lg transition-colors">
                <Twitter size={16} />
              </a>
              <a href="https://linkedin.com" target="_blank" rel="noreferrer" className="p-2 bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700 text-slate-300 hover:text-white rounded-lg transition-colors">
                <Linkedin size={16} />
              </a>
            </div>
          </div>
        ...
        <section className="space-y-4">
          <h2 className="text-lg font-bold tracking-tight border-l-2 pl-3" style={{ borderColor: activeAccent.hex }}>Skills & Tech Stack</h2>
          <div className="flex flex-wrap gap-2">
            {b.skills.map(s => (
              <span key={s} className={`px-3 py-1 rounded-lg text-xs font-semibold bg-slate-900 border border-slate-800 hover:border-slate-700 text-slate-300 transition-colors`}>
                {s}
              </span>
            ))}
          </div>
        ...
        {prefs.showProjects && (
          <section className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold tracking-tight border-l-2 pl-3" style={{ borderColor: activeAccent.hex }}>Featured Projects</h2>
              <span className="text-xs text-slate-400">Total Projects ({displayProjects.length})</span>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              {displayProjects.map(p => (
                <article key={p.id} className="p-6 bg-slate-900/30 border border-slate-800/80 rounded-xl hover:-translate-y-1 hover:border-slate-700 transition-all duration-300 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-2xl">{p.icon}</span>
                      <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded border ${activeAccent.badge}`}>
                        {p.status}
                      </span>
                    </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-800/60">
                    <div className="flex flex-wrap gap-1">
                      {p.stack.slice(0, 3).map(tech => (
                        <span key={tech} className="text-[10px] text-slate-300 bg-slate-800 px-2 py-0.5 rounded">
                          {tech}
                        </span>
                      ))}
                    </div>
        ...
        {prefs.showFlares && displayFlares.length > 0 && (
          <section className="space-y-4">
            <h2 className="text-lg font-bold tracking-tight border-l-2 pl-3" style={{ borderColor: activeAccent.hex }}>Latest Dev Thoughts</h2>
            <div className="space-y-3">
              {displayFlares.map(f => (
                <div key={f.id} className="p-5 bg-slate-900/20 border border-slate-850 rounded-xl space-y-3">
                  <p className="text-xs text-slate-300 leading-relaxed">{f.content}</p>
                  <div className="flex items-center justify-between text-[11px] text-slate-500">
                    <div className="flex items-center gap-3">
                      <span className="flex items-center gap-1"><Heart size={11} /> {f.likes} likes</span>
                      <span className="flex items-center gap-1"><MessageSquare size={11} /> {f.comments} replies</span>
                    </div>
                    <span>{f.ago}</span>
                  </div>
                </div>
        ...
        {prefs.showContact && (
          <section className="bg-slate-900/50 border border-slate-800/80 p-8 rounded-2xl space-y-6">
            <div className="text-center md:text-left">
              ...
              <p className="text-xs text-slate-400 mt-1">Fill out this secure form. Your inquiry will arrive directly in their DevLink notification inbox.</p>
            </div>
            <form onSubmit={handleContactSubmit} className="space-y-4 max-w-2xl mx-auto md:mx-0">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-1">
                  <label className="text-[11px] font-semibold text-slate-400 uppercase">Your Email *</label>
                  <input 
                    type="email" 
                    required 
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1">
                  <label className="text-[11px] font-semibold text-slate-400 uppercase">Hiring Role</label>
                  <select 
                    value={formData.role} 
                ...
                <div className="space-y-1">
                  <label className="text-[11px] font-semibold text-slate-400 uppercase">Budget Est. (USD, Optional)</label>
                  <input 
                    type="number" 
              ...
              <div className="space-y-1">
                <label className="text-[11px] font-semibold text-slate-400 uppercase">Message *</label>
                <textarea 
                  rows={4} 
                  required 
  ...
  const renderMinimalistWarm = () => (
    <div className="min-h-screen bg-[#f7f5f0] text-[#2c2a27] font-serif pb-20 border-t-8" style={{ borderTopColor: activeAccent.hex }}>
      <header className="border-b border-[#e2dec9] bg-[#f7f5f0]/80 backdrop-blur-md sticky top-0 z-30">
        <div className="max-w-3xl mx-auto px-4 py-6 flex items-center justify-between font-sans">
          <Link to="/dashboard" className="flex items-center gap-1.5 text-xs text-stone-500 hover:text-stone-900 transition-colors">
            <ArrowLeft size={14} /> Back to DevLink
          </Link>
      ...
      <main className="max-w-3xl mx-auto px-4 pt-16 space-y-20">
        ...
        <section className="space-y-6">
          <div className="flex flex-col items-center md:items-start md:flex-row gap-6">
            <img src={b.avatar} alt={b.name} className="w-24 h-24 rounded-full border border-stone-400 bg-stone-100 filter grayscale" />
            <div className="text-center md:text-left space-y-2">
              <h1 className="text-4xl font-normal tracking-tight font-serif text-[#1c1a18]">{b.name}</h1>
              <p className={`text-sm font-sans font-semibold tracking-wider uppercase ${activeAccent.text}`}>{b.role}</p>
          ...
          <div className="border-t border-b border-stone-300 py-6 my-2">
            <p className="text-base italic font-serif leading-relaxed text-[#403c37] max-w-2xl">{b.bio}</p>
          </div>
          <div className="flex justify-center md:justify-start items-center gap-3 pt-2 font-sans text-xs">
            <a href="https://github.com" target="_blank" rel="noreferrer" className="flex items-center gap-1 text-stone-600 hover:text-stone-950 hover:underline">
              <Github size={14} /> GitHub
            </a>
            <span>·</span>
            <a href="https://twitter.com" target="_blank" rel="noreferrer" className="flex items-center gap-1 text-stone-600 hover:text-stone-950 hover:underline">
              <Twitter size={14} /> Twitter
            </a>
            <span>·</span>
            <a href="https://linkedin.com" target="_blank" rel="noreferrer" className="flex items-center gap-1 text-stone-600 hover:text-stone-950 hover:underline">
        ...
        {/* Skills Section */}
        <section className="space-y-4">
          <h2 className="text-xs uppercase tracking-widest font-sans font-bold text-stone-400">Core Expertise</h2>
        ...
        {prefs.showProjects && (
          <section className="space-y-6">
            <h2 className="text-xs uppercase tracking-widest font-sans font-bold text-stone-400">Selected Works</h2>
            <div className="space-y-8">
              {displayProjects.map(p => (
                <div key={p.id} className="grid md:grid-cols-3 gap-4 border-b border-stone-200 pb-6 last:border-0">
                  <div className="md:col-span-1">
                    <span className="text-xl mb-1 block">{p.icon}</span>
                    <h3 className="text-base font-bold text-[#1c1a18]">{p.name}</h3>
                    <p className={`text-[10px] font-sans font-bold uppercase tracking-wider mt-1 ${activeAccent.text}`}>{p.status}</p>
                  </div>
                  <div className="md:col-span-2 space-y-4">
                    <p className="text-sm leading-relaxed text-stone-600 font-serif">{p.description}</p>
                    <div className="flex flex-wrap gap-2 font-sans text-[11px]">
                      {p.stack.map(tech => (
                        <span key={tech} className="px-2 py-0.5 bg-stone-200 rounded text-stone-700">
                          {tech}
                        </span>
                      ))}
                    </div>
        ...
        {prefs.showContact && (
          <section className="border-t-2 border-stone-300 pt-10 space-y-6">
            <div className="space-y-1">
              ...
              <p className="text-xs font-sans text-stone-500">Reach out for collaborations, full-time engineering placement, or consulting projects.</p>
            </div>
  ...
  const renderCyberpunk = () => (
    <div className="min-h-screen bg-black text-[#00ff66] font-mono pb-20 relative select-none">
      ...
      <header className="border-b border-[#00ff66] bg-black sticky top-0 z-30">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link to="/dashboard" className="text-xs text-[#00ff66] hover:text-[#ff00ea] transition-colors flex items-center gap-1">
            &lt;= DEVLINK_MAIN_SHELL
          </Link>
          <span className="text-xs animate-pulse text-[#ff00ea]">
            SYS_ONLINE // HOST:PORTFOLIO
          </span>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 pt-12 space-y-16">
        ...
        <section className="border-2 border-[#00ff66] p-6 shadow-[6px_6px_0px_rgba(0,255,102,0.2)] bg-black relative">
          ...
          <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start text-center sm:text-left">
            <img src={b.avatar} alt={b.name} className="w-24 h-24 border-2 border-[#00ff66] bg-[#111]" />
            <div className="space-y-3">
              <div>
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {b.skills.map(s => (
              <span key={s} className="px-2 py-1 text-xs border border-[#00ff66] hover:bg-[#00ff66] hover:text-black transition-colors duration-150 cursor-crosshair">
                [{s.toUpperCase()}]
              </span>
            ))}
          </div>
        ...
        {prefs.showProjects && (
          <section className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {displayProjects.map(p => (
                <div key={p.id} className="border border-[#00ff66] p-6 shadow-[4px_4px_0px_rgba(0,255,102,0.15)] hover:shadow-[6px_6px_0px_rgba(255,0,234,0.3)] hover:border-[#ff00ea] transition-all bg-black flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-center text-[10px]">
                      <span>{p.icon} DATA_NODE</span>
                      <span className="text-[#ff00ea] border border-[#ff00ea] px-1">{p.status}</span>
                    </div>
                    <h3 className="text-sm font-bold text-[#00ff66] mt-4">{p.name.toUpperCase()}</h3>
                    <p className="text-xs text-[#00ff66]/70 mt-2 line-clamp-3 leading-relaxed">{p.description}</p>
                  </div>
        ...
        {prefs.showContact && (
          <section className="border-2 border-[#00ff66] p-6 shadow-[6px_6px_0px_rgba(0,255,102,0.2)] bg-black space-y-6">
            <div>
              <h2 className="text-sm font-bold text-[#ff00ea] uppercase">[ // TERMINAL_COMMUNICATIONS ]</h2>
              <p className="text-[10px] text-[#00ff66]/80 mt-1">ESTABLISH PORTFOLIO CONGESTION TUNNEL TO HOST DIRECTLY.</p>
            </div>
  ...
  const renderGlass = () => (
    <div className="min-h-screen bg-[#070514] text-white pb-20 relative overflow-hidden font-sans">
      ...
      <header className="border-b border-white/10 bg-[#070514]/30 backdrop-blur-xl sticky top-0 z-30">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link to="/dashboard" className="text-xs text-white/60 hover:text-white transition-colors flex items-center gap-1">
      ...
      <main className="max-w-4xl mx-auto px-4 pt-12 space-y-16 relative z-10">
        ...
        <section className="bg-white/5 border border-white/15 p-8 rounded-2xl backdrop-blur-xl shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left">
          <div className="relative shrink-0">
            <img src={b.avatar} alt={b.name} className="w-28 h-28 rounded-full border-2 border-white/20 bg-white/10" />
            <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-4 border-[#070514] bg-success`} />
          </div>
          <div className="space-y-4 flex-1">
            <div>
            ...
            <div className="flex justify-center md:justify-start items-center gap-3 pt-2">
              <a href="https://github.com" target="_blank" rel="noreferrer" className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white/80 hover:text-white rounded-full transition-all">
                <Github size={15} />
              </a>
              <a href="https://twitter.com" target="_blank" rel="noreferrer" className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white/80 hover:text-white rounded-full transition-all">
                <Twitter size={15} />
              </a>
              <a href="https://linkedin.com" target="_blank" rel="noreferrer" className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white/80 hover:text-white rounded-full transition-all">
                <Linkedin size={15} />
              </a>
            </div>
          </div>
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {b.skills.map(s => (
              <span key={s} className="px-3.5 py-1.5 rounded-full text-xs bg-white/5 border border-white/10 backdrop-blur-md text-white/90 hover:bg-white/10 transition-colors">
                {s}
              </span>
            ))}
          </div>
        ...
        {prefs.showProjects && (
          <section className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {displayProjects.map(p => (
                <div key={p.id} className="p-6 bg-white/5 border border-white/10 hover:border-white/20 rounded-2xl backdrop-blur-md shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-2xl">{p.icon}</span>
                      <span className={`text-[9px] uppercase font-bold tracking-widest px-2.5 py-1 rounded-full bg-white/10 border border-white/20 ${activeAccent.text}`}>
                        {p.status}
                      </span>
                    </div>
                    <h3 className="text-sm font-bold text-white mt-4">{p.name}</h3>
                    <p className="text-xs text-white/70 mt-2 line-clamp-2 leading-relaxed">{p.description}</p>
                  </div>
                  <div className="mt-6 pt-4 border-t border-white/10 flex flex-wrap gap-1">
                    {p.stack.slice(0, 3).map(tech => (
                      <span key={tech} className="text-[10px] text-white/90 bg-white/5 px-2 py-0.5 rounded border border-white/5">
                        {tech}
                      </span>
                    ))}
                  </div>
        ...
        {prefs.showContact && (
          <section className="bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-xl space-y-6">
            <div>
              ...
              <p className="text-xs text-white/60 mt-1">Submit hiring inquiries or collaboration request tokens directly to {b.name}.</p>
            </div>
            <form onSubmit={handleContactSubmit} className="space-y-4 text-xs">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1">
  ...
  const renderSlate = () => (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-20">
      <header className="border-b border-slate-900 bg-slate-950/80 backdrop-blur-md sticky top-0 z-30">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link to="/dashboard" className="text-xs text-slate-400 hover:text-slate-200 transition-colors flex items-center gap-1.5">
            <ArrowLeft size={14} /> Return to App
          </Link>
          <div className="flex items-center gap-3">
            <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-md ${activeAccent.badge}`}>
              Active Portfolio
            </span>
          </div>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-4 pt-12 space-y-16">
        ...
        <section className="bg-slate-900 border border-slate-800 p-8 rounded-xl flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left shadow-lg">
          <div className="shrink-0">
            <img src={b.avatar} alt={b.name} className="w-24 h-24 rounded-lg border border-slate-700 bg-slate-950" />
          </div>
          <div className="space-y-4 flex-1">
            <div>
        ...
        <section className="space-y-4 bg-slate-900 border border-slate-800 p-6 rounded-xl">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Stack Expertise</h2>
          <div className="flex flex-wrap gap-2">
            {b.skills.map(s => (
              <span key={s} className="px-2.5 py-1 rounded bg-slate-950 border border-slate-800 text-xs text-slate-300">
                {s}
              </span>
            ))}
          </div>
        ...
        {prefs.showProjects && (
          <section className="space-y-6">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Featured Projects</h2>
            <div className="grid gap-4 md:grid-cols-2">
              {displayProjects.map(p => (
                <div key={p.id} className="p-6 bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-xl transition-all flex flex-col justify-between shadow-sm">
                  <div>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className={`text-[9px] uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-slate-950 border border-slate-800 ${activeAccent.text}`}>
                        {p.status}
                      </span>
                    </div>
        ...
        {prefs.showContact && (
          <section className="bg-slate-900 border border-slate-800 p-8 rounded-xl space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Submit inquiries directly to {b.name}. Leads are delivered to their DevLink dashboard.</p>
            </div>
            <form onSubmit={handleContactSubmit} className="space-y-4 text-xs">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1">
              ...
              <div className="space-y-6 overflow-y-auto pr-1">
                <div className="flex items-center justify-between">
                  <h3 className="text-base font-bold flex items-center gap-1.5">
                    <Sparkles size={16} className="text-primary animate-pulse" /> Portfolio Customizer
                  </h3>
                  <button 
                    onClick={() => setCustomizerOpen(false)}
                ...
                <div className="space-y-2">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">1. Layout Template</span>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <button
                        key={item.key}
                        onClick={() => setPrefs({...prefs, theme: item.key as PortfolioPrefs["theme"]})}
                ...
                {/* Accent selection */}
                <div className="space-y-2">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">2. Accent Color</span>
                  <div className="flex items-center gap-3">
                ...
                <div className="space-y-3 pt-2">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">3. Display Sections</span>
