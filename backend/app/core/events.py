        ...
        for handler in handlers:
            try:
                if asyncio.iscoroutinefunction(handler):
                    asyncio.create_task(self._safe_async_call(handler, event_name, **kwargs))
                else:
                    handler(**kwargs)
            except Exception as e:
                logger.error(f"Error executing handler {handler.__name__} for event {event_name}: {e}", exc_info=True)

    async def _safe_async_call(self, handler: Callable, event_name: str, **kwargs: Any) -> None:
        try:
            await handler(**kwargs)
        except Exception as e:
            logger.error(f"Error executing async handler {handler.__name__} for event {event_name}: {e}", exc_info=True)
